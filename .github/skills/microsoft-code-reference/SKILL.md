...
| Working code sample | `microsoft_code_sample_search` | `query: "upload blob managed identity", language: ""` |
...
microsoft_code_sample_search(query: "authenticate with managed identity", language: "")
...
- Package name doesn't follow conventions (`Azure.*` for .NET, `azure-*` for )
